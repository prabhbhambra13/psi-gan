...
def calc_and_save_metrics(model, x, z_bins, titles, out_dir, redshift):
    ...
    df_peak_counts.to_csv(f'{out_dir}/interpolate_z/results/z={redshift}_peak_counts.csv', mode='a', index=False, header=(not Path(f'{out_dir}/interpolate_z/results/z={redshift}_peak_counts.csv').is_file()))
    ...
    df_phase_distributions.to_csv(f'{out_dir}/interpolate_z/results/z={redshift}_phase_distributions.csv', mode='a', index=False, header=(not Path(f'{out_dir}/interpolate_z/results/z={redshift}_phase_distributions.csv').is_file()))
    ...
    for k1, k2 in list(combinations_with_replacement(k_values, 2))[2:]:
